            ...
            children: [
              ...
              Wrap(
                children: [
                  ...
                  TextButton(
                    onPressed: onEdit,
                    child: Text(requirement.status == JobStatus.pendingReview
                        ? 'Approve'
                        : 'Edit'),
                  ),
                  if (onReject != null)
...
class _EditRequirementDialogState extends State<EditRequirementDialog> {
  ...
  late final _salaryController = TextEditingController(
      text: widget.requirement.salaryAmount?.toString() ?? '');
  ...
  void initState() {
    ...
    if (widget.requirement.specificDays != null) {
      _specificDays.addAll(widget.requirement.specificDays!);
    }
  }
  ...
  bool get _scheduleValid {
    ...
    if (_isSpecificDays) {
      return _scheduleRepeat != null && _specificDays.isNotEmpty;
    }
    return false;
  }
          ...
          child: Column(
            ...
            children: [
              DropdownButtonFormField<String>(
                ...
                decoration:
                    const InputDecoration(labelText: 'Frequency of Care'),
              ...
              TextField(
                ...
                decoration: InputDecoration(
                  labelText: _frequency == FrequencyOfCare.daily
                      ? 'Salary (₹/day)'
                      : 'Salary (₹/month)',
                ),
              ...
              SegmentedButton<String>(
                segments: const [
                  ButtonSegment(
                      value: ScheduleType.dateRange, label: Text('Date Range')),
                  ButtonSegment(
                ...
                onSelectionChanged: (selection) => setState(() =>
                    _scheduleType = selection.isEmpty ? null : selection.first),
              ),
              if (_isDateRange) ...[
                const SizedBox(height: AppSpacing.sm),
                Row(
                ...
                if (_startDate != null &&
                    _endDate != null &&
                    _endDate!.isBefore(_startDate!))
                  const Text('End date must be on or after the start date',
              ...
              if (_isSpecificDays) ...[
                ...
                SegmentedButton<String>(
                  segments: const [
                    ButtonSegment(
                        value: ScheduleRepeat.weekly, label: Text('Weekly')),
                    ButtonSegment(
                        value: ScheduleRepeat.monthly, label: Text('Monthly')),
                  ],
                  selected: {if (_scheduleRepeat != null) _scheduleRepeat!},
                  emptySelectionAllowed: true,
                  onSelectionChanged: (selection) => selection.isEmpty
                      ? setState(() => _scheduleRepeat = null)
                      : _setScheduleRepeat(selection.first),
                ),
                if (_isWeekly) ...[
                  const SizedBox(height: AppSpacing.sm),
                  const Text(
                      'Select the days of the week needed (repeats every week)',
                  ...
                  Wrap(
                    ...
                    children: [
                      for (var weekday = 1; weekday <= 7; weekday++)
                        FilterChip(
                          label: Text(
                              ScheduleRepeat.weekdayAbbreviations[weekday]!),
                ...
                if (_isMonthly) ...[
                  const SizedBox(height: AppSpacing.sm),
                  const Text(
                      'Select the days of the month needed (repeats every month)',
                      style: TextStyle(color: AppColors.textSecondary)),
                  const SizedBox(height: AppSpacing.xs),
...
class _MonthCalendarPicker extends StatelessWidget {
  ...
  const _MonthCalendarPicker(
      {required this.selectedDays, required this.onToggle});
  ...
  Widget build(BuildContext context) {
    ...
    final leadingBlanks =
        DateTime(now.year, now.month, 1).weekday - 1; // Monday=1 -> 0 blanks
  ...
  switch (requirement.scheduleType) {
    ...
    case ScheduleType.specificDays:
      ...
      if (requirement.scheduleRepeat == ScheduleRepeat.weekly) {
        ...
        final names = sorted
            .map((d) => ScheduleRepeat.weekdayAbbreviations[d] ?? '$d')
            .join(', ');
        return 'Every: $names';
      }
            ...
            children: [
              ...
              _DetailRow(
                ...
                requirement.frequencyOfCare != null
                    ? FrequencyOfCare
                            .displayNames[requirement.frequencyOfCare] ??
